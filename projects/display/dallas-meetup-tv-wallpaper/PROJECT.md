...
### Desired outcome
**WebGL2** silk/fold moving gradient (dark black + grey, sparse white hint) at true **1920×1080**. Center **3-logo carousel** (Grok Bot → SpaceX → Cursor) as crisp SVG, fading one-at-a-time over a **120s** loop. Bottom-left white Universal Sans headline + subline (lab-editable). Reduced motion: still silk field + first logo + static text.

### Success signal
- Silk moving-gradient shader fills the 1920×1080 stage behind the lockup (WebGL2 on every browser); folds visibly drift while playing.
...
| --- | --- | --- |
| Background | WebGL2 silk moving gradient | Unicorn is look-only; remake without watermark; never block on WebGPU |
...
- [ ] Reduced motion / pause: still silk shader frame, Grok logo, static text
...
- [ ] Ground is a live WebGL2 silk shader on every load — visible motion when playing; no Unicorn SDK; no vgpu `init()` on this canvas; never a static black wash
