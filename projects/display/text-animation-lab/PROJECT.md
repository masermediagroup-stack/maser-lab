...
- Preview frame includes a **Fullscreen** control that expands the live animation to the viewport; fullscreen chrome is only a top-right **X** (plus Escape) to close
...
- [x] Detail preview can enter fullscreen; fullscreen UI is only a top-right X (plus Escape)
