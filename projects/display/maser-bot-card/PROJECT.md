...
| Stage bg | vgpu ground (demo **Background** knob, default `#F7F5F0`) + TL grey + quiet pointer cloud | behind the card; not on the card face, type, or mark |
...
- [ ] Ground / Background color is a demo knob (default `#F7F5F0`) and never sits on the card. Changing it does not tint the card fill, type, or mark.
