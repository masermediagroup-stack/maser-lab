...
- Build: One thin cuboid card (Three.js ExtrudeGeometry, no bevel). Type, mark, and sheen are painted on the face mesh (flat Display Trial). Tilt lives on that volume. The edge is the cuboid side, seen on tilt and flip. No outline rim, no bevel, no chrome edge, no second plane around the type or mark. Wordmark on Front v1. Live capsule stamped into the Back v1 lid map. CSS **ground** slate `#F7F5F0` behind the card (no vgpu field). Flip viewport is larger than the rest card (FlipNoClip).
...
| --- | --- | --- |
| Library | Thin Three.js cuboid (ExtrudeGeometry, bevel off) + type painted on lid maps + CSS slate ground | Type is on the cuboid face. The edge is the cuboid side. Rim, bevel, chrome, Geist, and a frame around the type are refused. No vgpu field on this card. |
...
| Sheen / light | Quiet wash **painted on the lid maps** only while pointer is on the card face; snap off on leave | no rest sheen, no idle center, no leftover specular, no inner-card / double-bezel mesh |
...
| Stage bg | CSS clean slate `#F7F5F0` | behind the card; not on the card face, type, or mark. No vgpu field. |
| Flip | One 180° turn on the cuboid, then settle (520ms, product state). Stage / viewport / camera expanded so corners + extrusion stay on the paper (FlipNoClip). Reduced: swap, no turn | Back on the wordmark, Front on the identity face. Text is the button. Swap type is black. No extra spin, no loop, no fade. |
...
- [ ] Reduced motion: no tilt, no sheen, ground still (`#F7F5F0` slate), mark planted `neutre`, no curl or pointer chase, face swap without 3D flip
- [ ] Card face is solid `#000`. Ground is CSS slate `#F7F5F0`. Capsule bleu `#3b93f0` on **Back v1**. Wordmark on Front v1.
- [ ] Ground is locked slate `#F7F5F0` (CleanSlateGround). No Stage-bg Calm/Interactive knobs. Ground never sits on the card fill, type, or mark.
- [ ] Front / Back turns the cuboid once to the other face and settles. Full card stays visible through the flip (FlipNoClip). Reduced motion swaps without the turn. Swap type is black.
