...
- [x] Mobile lab chrome is a compact hamburger that does not cover the hero headline or the meatball field (rendered). Open menu docks as a viewport-edge strip (top on ~390px, left rail on desktop) — not an overlay on the blobs. The WebGL field is the full viewport (`inset:0; width/height:100%`, backing store from client rect × DPR); the dock never becomes the canvas size. Panel stacks Hue / Saturation / Merge k / Wetness / Speed and scrolls if needed; Light/Dark/Replay/RM stay. Speed retunes weighted-arc travel only (not spawn rate, heat, or gate). No light-angle knob. Wetness 0 vs 1 is an obvious satin vs wet material change on Light and Dark (`uWetness` only; no center pin).
...
- Exact `k` (merge tension) defaults to `LMM_MERGE_K = 24`. Lab demo chrome may retune k/hue/sat/wetness/speed; product defaults stay locked. Speed is travel on the weighted arc (1 = locked pace). No per-ball facing spec.
