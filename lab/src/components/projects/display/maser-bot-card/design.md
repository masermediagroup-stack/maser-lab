...
2. Stage background **behind** the card, not on the card face, type, or mark: ground (demo knob, default `#000000`) + small grey gradient from the top-left, plus a small quiet cloud-type cursor shader that follows the pointer. Shader is **vgpu**. Not a new raw WebGL stack. Not the old dither wave. Ground never paints the card fill.
