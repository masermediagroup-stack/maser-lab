...
import type { BrandMedia } from "../types";
...
export function BeforeAfterSlider({
  ...
  return (
    <motion.div
      ref={trackRef}
      className={cn(
        "relative aspect-[4/3] w-full overflow-hidden border border-[var(--bcs-border)] bg-[var(--bcs-border)]",
      ...
      animate={
        visible || reduce ? { opacity: 1, scale: 1 } : { opacity: 0.92, scale: 0.985 }
      }
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          transition:
            reduce || dragging ? undefined : "clip-path 180ms cubic-bezier(0.22, 1, 0.36, 1)",
        }}
      >
        <Image
          src={before.src}
      ...
      <div
        ...
        className={cn(
          "bcs-ba-handle absolute top-1/2 z-20 flex size-11 -translate-x-1/2 -translate-y-1/2",
          "cursor-ew-resize items-center justify-center rounded-full border border-white/80 bg-white",
          "shadow-[0_4px_18px_rgba(0,0,0,0.18)] touch-none",
        )}
        style={{ left: `${position}%` }}
        onPointerDown={onPointerDown}
        onPointerMove={onPointerMove}
        ...
        onKeyDown={onKeyDown}
      >
        <span className="flex items-center gap-0.5 text-[var(--bcs-fg)]" aria-hidden>
          ‹ ›
        </span>
      </div>
    </motion.div>
