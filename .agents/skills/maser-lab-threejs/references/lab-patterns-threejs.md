...
| Thin extruded card cuboid | `display/maser-bot-card` | Rounded `ExtrudeGeometry`, `bevelEnabled: false`, type painted on lid maps (`MeshBasicMaterial` + `CanvasTexture`), cuboid side `MeshPhongMaterial` (black fill, satin catch). vgpu stage. Flip is one 180° turn (`LoopOnce`-style clamp). No Three.js GLSL. |
