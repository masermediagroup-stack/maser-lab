...
| Thin extruded card cuboid | `display/maser-bot-card` | Rounded `ExtrudeGeometry`, `bevelEnabled: false`, type painted on lid maps (`MeshBasicMaterial` + `CanvasTexture`), cuboid side `MeshPhongMaterial` (black fill, satin catch). CSS slate ground `#F7F5F0`. Flip is one 180° turn with expanded WebGL viewport (FlipNoClip). No Three.js GLSL. |
