...
Open dock is an **opaque strip** (`bg` lab token, no backdrop-blur, no scrim). Desktop: left rail (product **beside** it). Mobile: stacked top strip with `--lab-control-dock-top` on `.maser-lab` so the product sits **below** the strip — never a modal/drawer/sheet covering the field. Token writes must land on `.maser-lab` (that node owns `--lab-control-dock-*`; html inline vars lose).
