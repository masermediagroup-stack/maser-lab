...
| `DemoControlMenu` | Collapsible dock. Open: left rail (`sm+`); mobile knobs under the fold (scroll). Toggle: `Open demo menu` / `Close demo menu` |
...
Open dock is an **opaque strip** (`bg` lab token, no backdrop-blur, no scrim). Desktop (`sm+`): left rail (product **beside** it). Mobile: product owns the **first screen** (~100dvh). Open knobs live **under the fold** in document flow — scroll to them. Never a sticky footer that eats screen 1. Never `position: fixed` over the bottom of the first viewport. Never a modal/drawer/sheet. Token writes must land on `.maser-lab`. Choose left vs in-flow with the `640px` media query.

`--lab-control-dock-left` / `--lab-control-dock-top` / `--lab-control-dock-bottom` / `--lab-control-type-offset` pad **page copy** (`.lab-demo-inset`). Canvas/WebGL fields use `.lab-demo-field`: closed = full viewport; open desktop = remainder beside the rail; open mobile = **~100dvh first screen**, knobs in flow below (not a sticky footer). Never sit the field under a dimmed plate. Never `width/height: auto` on a replaced canvas (**300×150** trap). Never remount GL when the dock opens. Do not overlay chrome on product copy. Light/Dark only when that demo has two grounds. Replay / reduced motion stay in the existing shared row — do not add a second chrome cluster. Demos with no controls stay alone. Exception: `maser-dither-engine` keeps its own studio chrome — do not move, restyle, or transfer it.
